...
REM Source Link: https://github.com/git-for-windows/git/releases/
...
set "Link="
set "LinkForAllWindows32bit=https://github.com/git-for-windows/git/releases/download/v%Major%.%Minor%.%Build%.windows.%Revision%/Git-!SoftNameVersion!-32-bit.exe"
set "LinkForAllWindows64bit=https://github.com/git-for-windows/git/releases/download/v%Major%.%Minor%.%Build%.windows.%Revision%/Git-!SoftNameVersion!-64-bit.exe"

set "LinkFromGithub="
set "LinkFromDropbox="
set "LinkFromOneDrive="

set "SoftPath=%ProgramFiles%\Git"
...
git --version >nul 2>&1
if %errorlevel% equ 0 (
	echo Git has been successfully installed.&goto RunSD
) else (
	echo Failed to install Git. &exit
)

:RunSD
title _Hieuck.IT_'s Windows Application Starting Stable-Diffusion-Webui...
color 0B
mode con:cols=120 lines=17
if not exist "wget.exe" mode con:cols=80 lines=17
@cls
...
@echo.
@echo                 The current date and time are: %date% %time%
@echo                 Dang Khoi Chay Stable-Diffusion-Webui. Vui Long Cho...
@echo off
