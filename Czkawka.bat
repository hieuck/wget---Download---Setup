...
REM Source Link: https://github.com/qarmin/czkawka/releases/latest https://github.com/BtbN/FFmpeg-Builds/releases/latest
...
if exist "wget.exe" (
	wget --no-check-certificate --show-progress -q -O "FFmpeg.zip" "%LinkFFmpeg%"
) else (
...
if %size% equ 0 (
	if %sizeFFmpeg% equ 0 (
		echo %SoftName% and %NameFFmpeg% download failed. Both file sizes are 0KB. Downloading with browser...
		goto DLwB
	) else (
		echo %SoftName% download failed. File size is 0KB. Downloading with browser...
		goto DLwB
	)
) else if %sizeFFmpeg% equ 0 (
	echo %NameFFmpeg% download failed. File size is 0KB. Downloading with browser...
	goto DLwB
)
...
if %size% equ 0 (
	...
	start "" "%LinkFFmpeg%" /WAIT /D "%~dp0" /B "%NameFFmpeg%.zip"
)
...
if /i "%Extract7z%"=="Yes" (
	...
	@7z x "%NameFFmpeg%.zip" -o"%SoftPath%" -aoa -y %NameFFmpeg%\bin\ff*.exe
