...
if %ARCH%==x86 (
	set "CONFIG2021=https://raw.githubusercontent.com/hieuck/curl-uri-wget-download-setup/main/Setup/Configuration-2016-32.xml"
) else (
	set "CONFIG2021=https://raw.githubusercontent.com/hieuck/curl-uri-wget-download-setup/main/Setup/Configuration-2016-64.xml"
)
...
if exist "wget.exe" (
	...
	wget --no-check-certificate --show-progress -q -O "Configuration.xml" -U "%USERAGENT%" "%CONFIG2021%"
) else (
	curl -L --max-redirs 20 -A "%USERAGENT%" -o "%FILENAME%" "%LINK%" --insecure
	curl -L --max-redirs 20 -A "%USERAGENT%" -o "Configuration.xml" "%CONFIG2021%" --insecure || (
