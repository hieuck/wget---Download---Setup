...
set "SoftName=Pulover's Macro Creator"
set "Process=MacroCreator.exe"

set "FileName="
set "SoftNameVersion=5.4.1"
set "FileDLwB=MacroCreator*.exe"
...
if "%choice%"=="1" (
	...
	set "Link=https://udomain.dl.sourceforge.net/project/pulovers-macro-creator/MacroCreator-setup.exe"
	goto NextStepAfterChosen
) else if "%choice%"=="2" (
	REM My Github
	echo You have chosen to download from: %Menu2%
	set "Link=https://github.com/Pulover/PuloversMacroCreator/releases/download/v!SoftNameVersion!/MacroCreator-setup.exe"
	goto NextStepAfterChosen
) else if "%choice%"=="3" (
	REM My Dropbox
...
set "SoftPath=%ProgramFiles%\MacroCreator"
